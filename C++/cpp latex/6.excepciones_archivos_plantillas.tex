...
\subsection{Excepción de formato}
\hspace{0.55cm}Como se había visto previamente en este documento, creamos una \textbf{excepción} que controla el correcto flujo de \textbf{std::cin}, con esto nos referimos a que, en caso de que se le asigne a una variable double una cadena o carácter, el flujo de std::cin y del programa no se vea afectado y, como resultado, el programa continúe funcionando correctamente.

Sin embargo, esta excepción es más aplicada a entrada de valores a arreglos utilizando ciclos, no está contenida dentro de un bloque try-catch, en esta nueva sección, presentamos la excepción creada previamente, solamente que adaptada al bloque try-catch:
\begin{lstlisting}
   #include <iostream>
   ...
   int main() {
      ...
      try {
         // Entrada de datos a variables doubles.
         std::cout << "Ingresa un valor: "; std::cin >> a;
         if (std::cin.fail()) throw 1; // Si el flujo de cin tiene estado fail, lanza excepcion numérica.
         std::cout << "Ingresa un valor: "; std::cin >> a;
      ...
      // Excepción numérica.
         ...
         main() // Llamada recursiva a main (para este caso).
     }
   }
\end{lstlisting}
...
De igual forma, podemos instanciar el objeto \textit{ofstream} pasándole directamente el nombre y dirección del archivo a abrir:
\begin{center}
	\textit{ofstream Prueba("prueba.txt")}
\end{center}
