...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{git log:} enlista todas las instantáneas creadas de nuestro proyecto.
...
Vemos que en la \textit{Figura \ref{fig: 4}} y \textit{\ref{fig: 5}} se abre y ubica el directorio local de nuestro proyecto en VSC, en la \textit{Figura \ref{fig: 6}} se muestra un mensaje (puede aparecer o no) donde confirmamos que confiamos en este directorio, una vez cargado el directorio en VSC, debemos guardarlo como un \textit{área de trabajo}, en la \textit{Figura \ref{fig: 7}} se guarda como tal y en la \textit{Figura \ref{fig: 8}} se le da una ubicación y nombre al área de trabajo, puede estar ubicado en el mismo directorio local de nuestro proyecto y tener el mismo nombre, o no.
...
Entonces, debemos agregar \texttt{index.html} al área de ensayo, dándole seguimiento, con el comando \textbf{git add index.html}, una vez hecho esto, podemos realizar un primer commit (aunque el archivo esté vacío) usando el comando \textbf{git commit -m "Primer avance"}, como se ve en la \textit{Figura \ref{fig: 13}}.
\begin{figure}[H]
    \begin{center}
...
\subsubsection{Con la barra de tareas inferior azul}
\hspace{0.55cm}VSC posee una barra de tareas color azul en la parte inferior de su interfaz, donde suele mostrar los errores y alertas de un código, en que línea y columna estamos ubicados, entre otra información, en la zona izquierda de esta barra, encontramos un texto que nombra a la rama en donde estamos trabajando actualmente con nuestro proyecto en Git. Si damos clic sobre este texto, nos abre una barra de acciones en la parte superior central de VSC dándonos algunas opciones, como la de \textbf{Crear rama} y ver las ramas existentes, como se ve en la \textit{Figura \ref{fig: 17}}:
\begin{figure}[H]
	\begin{center}
...
Si realizamos alguna modificación en el archivo \texttt{index.html} y realizamos un commit nuevo, para subir este cambio a GitHub, presionamos alguna de las dos opciones mostradas en la \textit{Figura \ref{fig: 28}} para subir nuestros cambios, a esta acción se le conoce como \textbf{push}:
\begin{figure}[H]
    \begin{center}
...
Sabemos que se pueden editar archivos en GitHub, si en nuestro archivo \texttt{index.html} le agregamos un parráfo desde GitHub y queremos que aparezca en VSC, simplemente debemos darle clic al ícono de \textbf{sincronización} en la \textbf{barra de tareas} (\textit{Figura \ref{fig: 32}}). Para que aparezca el cambio de VSC a GitHub, realizamos el commit con nuestros cambios y realizamos un push a el repositorio que deseemos:
\begin{figure}[H]
    \begin{center}
        \caption{Subir proyecto a GitHub en un repositorio existente}
        \label{fig: 32}
...
\section{Fork}
\hspace{0.55cm}Si recordamos el concepto de \textit{ramas}, sabemos que es la copia de los archivos de nuestro proyecto para que más de una persona pueda trabajar en nuestro repositorio, al final, se mezclan ambas ramas para que se unifique el proyecto, entonces, el concepto de \textbf{fork} va más allá de una rama, sigue la misma lógica, pero a escala de repositorios. Un \textit{fork} es la copia del repositorio de un usuario en nuestra cuenta o cuenta de algún usuario que se haya visto interesado en el repositorio que busca copiar, el usuario original puede realizar commits y modificaciones en su trabajo original, mientras que el usuario que tiene una copia del repositorio puede clonarlo y hacer lo mismo, el usuario con el repositorio fork puede realizar una solicitud de merge al usuario propietario y este puede aceptarla, comentarla, revisarla, compararla o declinarla, como se ve en la \textit{Figura \ref{fig: 35}}:
\begin{figure}[H]
    \begin{center}
        \caption{Ubicación del botón Fork en GitHub}
        \label{fig: 35}
        \includegraphics[width=14cm]{conceptos/fork1.jpg}
    \end{center}
\end{figure}

Si vamos a un repositorio nuestro en GitHub en la esquina superior derecha, aparece un botón con el texto \textbf{Fork}, como en la \textit{Figura \ref{fig: 36}}) y \textit{\ref{fig: 37}}:
\begin{figure}[H]
    \begin{center}
        \caption{Ubicación del botón Fork en GitHub}
        \label{fig: 36}
