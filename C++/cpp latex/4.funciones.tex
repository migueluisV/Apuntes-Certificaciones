...
\hspace{0.55cm}Una función o procedimiento también \textbf{arreglos} como argumento, basta con indicarle al argumento un tipo de dato, un nombre identificativo y los corchetes (sin ningún contenido dentro de ellos).\begin{center}\textit{void DespliegueArreglo(int arreglo[], int size) \{\}}\end{center}
...
\begin{lstlisting}
    ...
    int main()
    {
        ...
        Funcion(&variable); // Le pasamos la referencia de variable local, la función le asigna el valor "100" y después se despliega.
        cout << variable;
...
\section{Constante PI}
\hspace{0.55cm}En algunos otros lenguajes, podemos utilizar algunas funciones matemáticas solamente agregando alguna biblioteca a nuestra solución, dicha biblioteca posee algunos valores constantes matemáticos, como el valor Euler o PI, sin embargo, ninguna biblioteca en C++ posee el valor de PI.

En vez de declarar una variable float, double o long constante, podemos utilizar la siguiente instrucción que viene integrada en las versiones más recientes de C++:
\begin{center}
	\textit{\# define \_ USE\_ MATH\_ DEFINES // Instrucción que da acceso a un valor constante de PI.}
\end{center}

Con esa definición establecida, podemos acceder al valor de PI mediante la variable M\_ PI, un ejemplo de su uso puede ser:
\begin{lstlisting}
   #define _USE_MATH_DEFFNES
   #include <iostream>

   int main(){
      std::cout << M_PI << std::endl; // Despliega el valor de Pi.
   }  
\end{lstlisting}