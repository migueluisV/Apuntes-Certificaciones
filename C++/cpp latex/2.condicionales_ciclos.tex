...
\begin{center}
	\textit{
		...
		acumulador = acumulador + 10\\acumulador += 5; // Forma de acumulador con operadores de asignación. \\
		acumulador += 10;
	}
\end{center}
\begin{lstlisting}
    int main()
    {
        ...
        while (x<10) { // Condicional, este ciclo imprime el contador que se está evaluando hasta que sea 9, el contador va incrementando uno a uno, pero se puede modificar este incremento.
...
\textit{Nota}: las condicionales switch no pueden ser utilizadas con tipos de datos string y char, esto es debido a que C++ no comprende del todo las cadenas, caracteres y secuencias de caracteres, por lo que le resulta complicado o confuso el hecho de evaluar una cadena con distintos valores.



\section{Operadores lógicos}
\hspace{0.55cm}Los operadores lógicos usados para condicionales y ciclos son se encuentran en la \textit{Tabla \ref{tab: 6}}:
\begin{table}[H]
    \begin{center}
